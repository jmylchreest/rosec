...
| `theme_file` | string or omitted | _(none)_ | Path to an external TOML theme file. If relative, resolved from the directory containing `config.toml`. When set, the inline `[prompt.theme]` table is ignored. See [External theme file](#external-theme-file). |
...
| `danger_color` | `dc` | `#f38ba8ff` | Danger/warning color (e.g. expiring TOTP digits) |
...
Instead of defining colors inline, you can point `theme_file` at a separate TOML
file. This is useful for themes generated by tools like
[tinct](https://github.com/jmylchreest/tinct) or for sharing a theme across
machines.

The file must contain a `[prompt.theme]` table — the same keys and aliases
accepted inline work here. Any fields not present in the file fall back to
compiled-in defaults. When `theme_file` is set, the inline `[prompt.theme]`
table in `config.toml` is ignored entirely.

Relative paths are resolved from the directory containing `config.toml`
(typically `~/.config/rosec/`).

```toml
# config.toml
[prompt]
theme_file = "tinct-theme.toml"
```

```toml
# ~/.config/rosec/tinct-theme.toml
[prompt.theme]
bg   = "#1a1b26cc"
fg   = "#c0caf5ff"
ac   = "#7aa2f7ff"
...
[prompt]
backend = "builtin"
# theme_file = "tinct-theme.toml"  # load theme from external file
