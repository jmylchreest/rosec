...
use rosec_core::{
    AttributeDescriptor, Attributes, Capability, Fido2CredentialMeta, Fido2KeyMaterial,
    ItemAttributes, ItemMeta, ItemType, ItemUpdate, NewItem, Provider, ProviderCallbacks,
    ProviderError, ProviderStatus, SecretBytes, SshKeyMeta, SshPrivateKeyMaterial, UnlockInput,
    attributes_match,
};
use tokio::fs;
...
fn load_metadata_sidecar(path: &Path) -> Option<Vec<MetaItem>> {
    let content = match std::fs::read(path) {
        ...
        Err(e) => {
            warn!(path = %path.display(), error = %e, "metadata sidecar read failed");
            return None;
        }
    };
...
pub struct LocalVault {
    ...
    /// Sidecar metadata, loaded at construction; refreshed after each save
    /// (and after unlock if it was missing).  Lets `list_items` answer
...
impl LocalVault {
    ...
    fn write_sidecar(&self, items: &[VaultItemData]) {
        let sidecar = MetaSidecar::from_items(items);
    ...
    async fn save(&self) -> Result<(), ProviderError> {
        ...
        // Vault-first ordering: a crash here leaves the sidecar stale, not
        // ghost-pathed.  Sidecar errors are warn-logged, not fatal.
        self.write_sidecar(&state.data.items);
    ...
    fn meta_item_to_locked_meta(&self, item: MetaItem, provider_id: &str) -> ItemMeta {
        let mut attributes: Attributes = item
            .attribute_hashes
            .keys()
        ...
        ItemMeta {
            ...
            locked: true,
            attribute_hashes: Some(item.attribute_hashes),
        }
    }
...
impl Provider for LocalVault {
    ...
    async fn unlock(&self, input: UnlockInput) -> Result<(), ProviderError> {
        ...
        // Populate sidecar on first unlock if it wasn't already loaded.
        // Closes the cold-start gap where SearchItems would return empty
        // until the first item write.
        let needs_sidecar = self
            .metadata_cache
            .read()
            .map(|c| c.is_none())
            .unwrap_or(true);
        if needs_sidecar {
            self.write_sidecar(&data.items);
        }
    ...
    async fn list_ssh_keys(&self) -> Result<Vec<SshKeyMeta>, ProviderError> {
        ...
        Ok(state
            ...
            .items
            .iter()
            .filter(|item| crate::ssh::is_ssh_key_item(item))
            .map(|item| crate::ssh::item_to_ssh_key_meta(item, &self.id))
            .collect())
    }
    ...
    async fn get_ssh_private_key(&self, id: &str) -> Result<SshPrivateKeyMaterial, ProviderError> {
        ...
        let item = state
            ...
            .find(|item| item.id == id)
            .ok_or(ProviderError::NotFound)?;
        crate::ssh::item_ssh_private_key(item)
    }
    ...
    async fn list_fido2_credentials(&self) -> Result<Vec<Fido2CredentialMeta>, ProviderError> {
        ...
        Ok(state
            ...
            .items
            .iter()
            .filter(|item| crate::fido2::is_fido2_item(item))
            .filter_map(|item| crate::fido2::item_to_fido2_meta(item, &self.id))
            .collect())
    }
    ...
    async fn get_fido2_key(
        ...
        let item = state
            ...
            .ok_or(ProviderError::NotFound)?;
        crate::fido2::item_fido2_key(item, credential_id)
    }
