...
impl Config {
    /// If `prompt.theme_file` is set, load the external theme file and replace
    /// the inline theme with its contents.  `config_dir` is the directory
    /// containing the main config.toml — relative `theme_file` paths are
    /// resolved against it.
    ///
    /// Missing or unparseable theme files are logged as warnings; the
    /// compiled-in defaults are used instead.
    pub fn resolve_theme_file(&mut self, config_dir: &Path) {
        let Some(ref rel) = self.prompt.theme_file else {
            return;
        };

        let theme_path = config_dir.join(rel);
        let content = match std::fs::read_to_string(&theme_path) {
            ...
            Err(e) => {
                tracing::warn!(
                    ...
                    error = %e,
                    "could not read theme_file, using defaults",
                );
                return;
            }
        };

        match toml::from_str::<ThemeFileWrapper>(&content) {
            Ok(wrapper) => {
                tracing::info!(path = %theme_path.display(), "loaded external theme");
                self.prompt.theme = wrapper.prompt.theme;
            }
            Err(e) => {
                tracing::warn!(
                    path = %theme_path.display(),
                    error = %e,
                    "could not parse theme_file, using defaults",
                );
            }
        }
    }
...
pub struct PromptConfig {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Optional path to an external theme file (e.g. generated by tinct).
    ///
    /// If relative, resolved relative to the directory containing config.toml
    /// (typically `$XDG_CONFIG_HOME/rosec/`).  The file must contain a
    /// `[prompt.theme]` table.  Any fields not present in the external file
    /// fall back to compiled-in defaults.
    ///
    /// When set, the inline `[prompt.theme]` table in config.toml is ignored.
    #[serde(default)]
...
/// Wrapper used when deserializing an external theme file that contains
/// a `[prompt.theme]` table (e.g. tinct-generated files).
#[derive(Debug, Clone, Default, Deserialize)]
struct ThemeFileWrapper {
    #[serde(default)]
    prompt: ThemeFilePrompt,
}

#[derive(Debug, Clone, Default, Deserialize)]
struct ThemeFilePrompt {
    #[serde(default)]
    theme: PromptTheme,
}
...
mod tests {
    ...
    #[test]
    fn resolve_theme_file_loads_external_theme() {
        let dir = tempfile::tempdir().unwrap();
        let theme_path = dir.path().join("tinct-theme.toml");
        std::fs::write(
            &theme_path,
            r##"
                [prompt.theme]
                background = "#aabbccdd"
                accent_color = "#11223344"
            "##,
        )
        .unwrap();
        ...
        cfg.resolve_theme_file(dir.path());
        assert_eq!(cfg.prompt.theme.background, "#aabbccdd");
        assert_eq!(cfg.prompt.theme.accent_color, "#11223344");
        // Fields not in the theme file should get compiled-in defaults.
        assert_eq!(cfg.prompt.theme.foreground, default_color_foreground());
        assert_eq!(cfg.prompt.theme.font_size, default_font_size());
    }

    #[test]
    fn resolve_theme_file_missing_falls_back_to_defaults() {
        let dir = tempfile::tempdir().unwrap();
        let mut cfg: Config = toml::from_str(
            r#"
                [prompt]
                theme_file = "nonexistent.toml"
            "#,
        )
        .unwrap();

        cfg.resolve_theme_file(dir.path());
        // Theme should remain the serde-deserialized default (all defaults).
        assert_eq!(cfg.prompt.theme.background, default_color_background());
    }

    #[test]
    fn no_theme_file_keeps_inline_theme() {
        ...
        assert_eq!(cfg.prompt.theme.background, "#deadbeef");
        assert!(cfg.prompt.theme_file.is_none());
    }
}
