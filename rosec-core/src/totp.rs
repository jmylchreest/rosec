...
fn parse_otpauth_uri(uri: &str) -> Result<TotpParams, TotpError> {
    ...
    for (key, value) in parsed.query_pairs() {
        match key.as_ref() {
            "secret" => secret_str = Some(value.into_owned()),
            "algorithm" | "algo" => {
...
fn decode_base32(input: &str) -> Result<Zeroizing<Vec<u8>>, TotpError> {
    ...
    let padded = format!("{cleaned}{}", "=".repeat(pad_len));
...
pub fn generate_code(params: &TotpParams, time: SystemTime) -> String {
    let unix_secs = time
    ...
    let hash = match params.algorithm {
        TotpAlgorithm::Sha1 => {
            let mut mac =
                Hmac::<Sha1>::new_from_slice(&params.secret).expect("HMAC accepts any key length");
...
pub fn generate_code_now(params: &TotpParams) -> String {
...
mod tests {
    ...
    fn rfc6238_sha256_test_vectors() {
        ...
        let t = UNIX_EPOCH + Duration::from_secs(59);
        assert_eq!(generate_code(&params, t), "46119246");

        let t = UNIX_EPOCH + Duration::from_secs(1111111109);
        assert_eq!(generate_code(&params, t), "68084774");

        let t = UNIX_EPOCH + Duration::from_secs(1234567890);
        assert_eq!(generate_code(&params, t), "91819424");

        let t = UNIX_EPOCH + Duration::from_secs(2000000000);
        assert_eq!(generate_code(&params, t), "90698825");
    }
    ...
    fn rfc6238_sha512_test_vectors() {
        ...
        let t = UNIX_EPOCH + Duration::from_secs(59);
        assert_eq!(generate_code(&params, t), "90693936");

        let t = UNIX_EPOCH + Duration::from_secs(1111111109);
        assert_eq!(generate_code(&params, t), "25091201");

        let t = UNIX_EPOCH + Duration::from_secs(1234567890);
        assert_eq!(generate_code(&params, t), "93441116");

        let t = UNIX_EPOCH + Duration::from_secs(2000000000);
        assert_eq!(generate_code(&params, t), "38618901");
    }
    ...
    fn six_digit_code_is_zero_padded() {
        ...
        let t = UNIX_EPOCH + Duration::from_secs(1111111109);
        let code = generate_code(&params, t);
